...
let Brie = Eztrvds[0]              //jsBridge
let amt = Eztrvds[1]     //amount
let ren = Eztrvds[2]      //currency
let OpWin = Eztrvds[3]      //openWindow
...
internal class DizunVcuViewController: UIViewController,WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {

    var dkizo: Nixyye?
    var chuaosn: WKWebView?
    ...
    override func viewDidLoad() {
        ...
        if dkizo!.qiozix != nil {
            view.backgroundColor = UIColor.init(hexString: dkizo!.qiozix!)
        }
        
        AppsFlyerLib.shared().appsFlyerDevKey = dkizo!.detzt!
        AppsFlyerLib.shared().appleAppID = dkizo!.mcpaos!
...
//        
        ...
        Eztrvds = dkizo!.foozuy!.components(separatedBy: ",")
//        HuntOrderKrajs = [aTc,diaChon, diyicicho, hdrawo, geicho, chozh, diyichCha, deop]
        let usrScp = WKUserScript(source: dkizo!.bxaiox!, injectionTime: .atDocumentEnd, forMainFrameOnly: true)
        ...
        chuaosn = WKWebView(frame: .zero, configuration: cofg)
        chuaosn!.allowsBackForwardNavigationGestures = true
        chuaosn?.uiDelegate = self
        chuaosn?.navigationDelegate = self
        view.addSubview(chuaosn!)
        
        kaoieus = dkizo!.weizuz!
        if kaoieus?.contains("://") == true {
            chuaosn?.load(URLRequest(url:URL(string: kaoieus!)!))
        } else {
            chuaosn?.load(URLRequest(url:URL(string: Kixtcfc(kaoieus!)!)!))
        }
    ...
    override func viewDidLayoutSubviews() {
        ...
        if let ws = UIApplication.shared.connectedScenes.first as? UIWindowScene,
           let statusBarManager = ws.statusBarManager {
            ...
            let statusBarHeight = dkizo!.lxipo!.contains("V") ? statusBarManager.statusBarFrame.height : 0
            let bottomHeight = dkizo!.lxipo!.contains("I") ? view.safeAreaInsets.bottom : 0
            chuaosn?.frame = CGRectMake(0, statusBarHeight, view.bounds.width, view.bounds.height - statusBarHeight - bottomHeight)
        }
    }
